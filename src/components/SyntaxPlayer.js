...
export default class Player extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (show.number !== prevProps.show.number) {
            ...
            localStorage.setItem(`lastPlayed${show.number}`, JSON.stringify({ lastPlayed: currentTime }))
            localStorage.setItem(`lastVolumeSetting`, JSON.stringify({ lastVolumePref: currentVolume }))
            localStorage.setItem(`lastPlaybackSetting`, JSON.stringify({ lastPlaybackRate: playbackRate }))
        }
    }
    ...
    scrubTime = eventData => (eventData.nativeEvent.offsetX / this.progress.offsetWidth) * this.audio.duration
        ...
        return (
            <div className="player">
                <div className="player__section player__section--left">
                    <button onClick={this.togglePlay} aria-label={playing ? 'pause' : 'play'} type="button">
                        <p className="player__icon">{playing ? <FiPause /> : <FiPlay />}</p>
                        <p>
